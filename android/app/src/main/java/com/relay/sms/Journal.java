...
final class Journal {
    private static SharedPreferences prefs(Context context) { return context.getSharedPreferences("sms-journal", Context.MODE_PRIVATE); }
    static synchronized String deviceId(Context context) {
        String id = prefs(context).getString("deviceId", null);
        if (id == null) {
            id = UUID.randomUUID().toString();
            if (!prefs(context).edit().putString("deviceId", id).commit()) throw new IllegalStateException("Cannot persist device identity.");
        }
        return id;
    }
    ...
    static synchronized void put(Context context, JSONObject record) {
        if (!prefs(context).edit().putString("pending", record.toString()).commit()) throw new IllegalStateException("Cannot persist SMS attempt. Nothing will be sent.");
    }
    static synchronized void acknowledge(Context context, String id, int part, int code) throws Exception {
    ...
    static synchronized void clear(Context context) {
        if (!prefs(context).edit().remove("pending").commit()) throw new IllegalStateException("Could not clear confirmed attempt.");
    }
}
