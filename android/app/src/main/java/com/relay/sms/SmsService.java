...
public final class SmsService extends Service {
    ...
    private Notification notification(String text, boolean ongoing) {
        return notification(text, ongoing, 0, 0);
    }
    ...
    private Notification notification(String text, boolean ongoing, int done, int total) {
        ...
        Notification.Builder builder =
                new Notification.Builder(this, "sending")
            ...
            builder.addAction(
                    new Notification.Action.Builder(
                                    ...
                                    PendingIntent.getService(
                                            this,
                                            1,
                                            new Intent(this, SmsService.class).setAction("STOP"),
    ...
    private void runCampaign(int subscription) {
        PowerManager.WakeLock wake =
                getSystemService(PowerManager.class)
                        .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "RelaySMS:Campaign");
        int sent = 0;
        ...
        try {
            ...
            if (subscription < 0
                ...
                throw new IllegalStateException("SMS permission or SIM unavailable");
            JSONObject pending = Journal.pending(this);
            if (pending != null) {
                String result = Journal.outcome(pending);
                report(pending, result == null ? "uncertain" : result);
            }
            SmsManager sms =
            ...
            int total = 0;
            JSONArray initial = LocalStore.load(this).getJSONArray("rows");
            for (int i = 0; i < initial.length(); i++)
                if (initial.getJSONObject(i).optString("status").equals("queued")) total++;
            while (!cancelled) {
                JSONObject claim = LocalStore.claim(this, UUID.randomUUID().toString());
                if (claim == null) {
                    finalText = getString(R.string.notif_done);
                    break;
                }
                ...
                JSONObject record =
                        new JSONObject()
                                ...
                                .put("attemptId", claim.getString("attemptId"))
                                .put("codes", codes);
                    ...
                    Intent event =
                            new Intent(this, SmsSentReceiver.class)
                                    .setData(
                                            Uri.parse(
                                                    "relay://sms/"
                                                            + claim.getString("attemptId")
                                                            + "/"
                                                            + i))
                                    .putExtra("attemptId", claim.getString("attemptId"))
                                    .putExtra("part", i);
                    callbacks.add(
                ...
                if (parts.size() == 1)
                    ...
                    sms.sendMultipartTextMessage(
                            claim.getString("phone"), null, parts, callbacks, deliveries);
                ...
                if (!outcome.equals("sent")) {
                    finalText = getString(R.string.notif_paused);
                    break;
                }
                sent++;
                notifications.notify(
                        1,
                        notification(
                                getString(R.string.notif_progress, sent, total),
                                true,
                                sent,
                                total));
                Thread.sleep(3000);
            ...
            try {
                ...
                /* Durable journal stays for the next start. */
            }
            ...
            if (Build.VERSION.SDK_INT < 33
                    || checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)
                            == PackageManager.PERMISSION_GRANTED)
                notifications.notify(2, notification(finalText, false));
            stopSelf();
        }
    }
