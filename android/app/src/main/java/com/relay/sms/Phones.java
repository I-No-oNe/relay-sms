...
final class Phones {
    static String country(Context context) {
        ...
        if ((iso == null || iso.isEmpty()) && telephony != null)
    ...
    static String e164(String raw, String country) {
        ...
        if (result == null && !value.startsWith("+"))
            result =
                    PhoneNumberUtils.formatNumberToE164(
                            "+" + value.replaceAll("[^0-9]", ""), country);
        return result;
    }

    static String normalize(String raw, String country) {
        String result = e164(raw, country);
        return result == null ? raw.trim() : result;
    }

    static boolean valid(String raw, String country) {
        return e164(raw, country) != null;
    }

    static String display(String phone, String country) {
        String result = PhoneNumberUtils.formatNumber(phone, country);
        return result == null ? phone : result;
    }
}
