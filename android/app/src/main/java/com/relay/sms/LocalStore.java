...
final class LocalStore {
    private static AtomicFile file(Context context) {
        return new AtomicFile(new File(context.getFilesDir(), "campaign.json"));
    }
        ...
        if (!file.getBaseFile().exists())
            return new JSONObject()
                    ...
                    .put(
                            "variants",
                            new JSONArray()
                                    .put(context.getString(R.string.default_variant_1))
                                    .put(context.getString(R.string.default_variant_2))
                                    .put(context.getString(R.string.default_variant_3)));
    ...
    // Marks the next queued row as sending in one locked read-write, so delivery reports saved
    // meanwhile aren't overwritten.
    static synchronized JSONObject claim(Context context, String attemptId) throws Exception {
        ...
        for (int i = 0; i < rows.length(); i++)
            if (rows.getJSONObject(i).optString("status").equals("queued")) {
                JSONObject row =
                        rows.getJSONObject(i).put("status", "sending").put("attemptId", attemptId);
                save(context, data);
    ...
    static synchronized void delivery(
            Context context, String attemptId, int part, int parts, String outcome)
            throws Exception {
        JSONObject data = load(context);
        JSONArray rows = data.getJSONArray("rows");
        for (int i = 0; i < rows.length(); i++) {
            JSONObject row = rows.getJSONObject(i);
            if (!row.optString("attemptId").equals(attemptId)) continue;
            if (outcome.equals("undelivered")) row.put("status", "undelivered");
            else if (!row.optString("status").equals("undelivered")) {
                JSONArray done = row.optJSONArray("deliveredParts");
                if (done == null) row.put("deliveredParts", done = new JSONArray());
                boolean seen = false;
                for (int k = 0; k < done.length(); k++) seen |= done.getInt(k) == part;
    ...
    static synchronized void result(Context context, int row, String outcome) throws Exception {
        JSONObject data = load(context);
        data.getJSONArray("rows").getJSONObject(row).put("status", outcome);
        save(context, data);
    }
}
