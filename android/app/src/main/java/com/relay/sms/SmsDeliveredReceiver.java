...
public final class SmsDeliveredReceiver extends BroadcastReceiver {
    @Override public void onReceive(Context context, Intent intent) {
        ...
        try { LocalStore.delivery(context, intent.getStringExtra("attemptId"), intent.getIntExtra("part", 0), intent.getIntExtra("parts", 1), outcome); }
        catch (Exception ignored) { /* The row stays "sent". */ }
    }
}
