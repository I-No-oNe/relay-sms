...
final class ContactRules {
    ...
    static boolean nameOk(String name, String script) {
        ...
        return value.matches("[\\p{Is" + script + "}\\p{M} .'\\-\u05F3\u05F4]+")
                && value.matches(".*\\p{Is" + script + "}.*");
    }
    ...
    static String cleanName(String raw) {
        return raw.replaceAll("[\\p{So}\\p{Sk}\\p{Cf}\\p{Cs}\\p{Co}\\uFE0E\\uFE0F\\u20E3]", " ")
                .replaceAll("\\s+", " ")
                .trim();
    }

    static String render(String text, String name) {
